...
def binary_addition(inputA, inputB):
    result = ""
    remind = 0
...
def sha1_algorithm(input):
    ...
    array_ascii = string_to_ascii(input)

    #Step 2: Convert ASCII array to binary array
    array_binary = dec_to_binary(array_ascii)
    ...
    len_array_binary_in_bit = dec_to_binary(temp_len_in_dec)
    ...
    for j in range(0, len(array_binary_32bit)):
        for i in range(16, 80, 1):
            ...
            xorA = xor_bitwise(wordA, wordB)
            xorB = xor_bitwise(xorA, wordC)
            xorC = xor_bitwise(xorB, wordD)

            newWord = binary_left_rotate(xorC, 1)
    ...
    for i in range(len(array_binary_32bit)):
        ...
        for j in range(len(array_binary_32bit[i])):
            ...
            if j < 20:
                b_and_c = and_bitwise(b, c)
                not_b = not_bitwise(b)
                not_b_and_d = and_bitwise(not_b, d)
                f = or_bitwise(b_and_c, not_b_and_d)
                k = "01011010100000100111100110011001"
            elif j < 40:
                b_xor_c = xor_bitwise(b, c)
                f = xor_bitwise(b_xor_c, d)
                k = "01101110110110011110101110100001"
            elif j < 60:
                b_and_c = and_bitwise(b, c)
                b_and_d = and_bitwise(b, d)
                c_and_d = and_bitwise(c, d)
                bc_or_bd = or_bitwise(b_and_c, b_and_d)
                f = or_bitwise(bc_or_bd, c_and_d)
            ...
            temp_add = binary_addition(binary_left_rotate(a, 5), f)
            temp_add = binary_addition(temp_add, e)
            temp_add = binary_addition(temp_add, k)
            temp_add = binary_addition(temp_add, array_binary_32bit[i][j])
            temp_add = truncate(temp_add, 32)
            #swap around
            e = d
            d = c
            c = binary_left_rotate(b, 30)
            b = a
            a = temp_add

        # Step 14: add initialized variables 'h_n' with new coressponding words
        h0 = truncate(binary_addition(h0, a), 32)
        h1 = truncate(binary_addition(h1, b), 32)
        h2 = truncate(binary_addition(h2, c), 32)
        h3 = truncate(binary_addition(h3, d), 32)
        h4 = truncate(binary_addition(h4, e), 32)

    #Step 15: join every h_n and convert from binary to hex
    result = binary_to_hex(h0 + h1 + h2 + h3 + h4)
    return result
